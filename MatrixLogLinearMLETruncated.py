...
class MatrixLogLinearMLETruncated(object):
    """Object that runs approximate maximum likelihood parameter training."""

    def __init__(self, baseModel, bpIters):
    ...
    def addData(self, states, features):
        ...
        for k in range(model.num_edges):
            ...
            #pairwise[states[model.var_list[i-1]], states[model.var_list[j-1]]] += 1
            pairwise[states[var], states[neighbor]] += 1

        example = np.append(labeled_feature_mat.flatten(), pairwise.flatten())

        self.models.append(model)
        self.beliefPropagators.append(MatrixBeliefPropagatorTruncated(model))
    ...
    def getFeatureExpectations(self, beliefPropagators):
        ...
        for i in range(len(self.labels)):
            ...
            bp.runInference(display = 'off', maxIter = self.bpIters)
            bp.computeBeliefs()
    ...
    def dualObjective(self, weightVector):
        """Compute the learning objective with the provided weight vector."""
        self.setWeights(weightVector, self.models)
        featureExpectations = self.getFeatureExpectations(self.beliefPropagators)
        ...
        # add regularization penalties
        dObjective += self.l1Regularization * np.sum(np.abs(weightVector))
        dObjective += 0.5 * self.l2Regularization * np.dot(weightVector, weightVector)

        # add likelihood penalty
        dObjective -= np.dot(weightVector, self.featureSum / len(self.labels))

        for bp in self.beliefPropagators:
            dObjective += bp.computeDualObjective() / len(self.labels)
            # print "Finished one inference"
