...
class AutogradImageSegmentation(object):
    ...
    def learn_primal(self):
        ...
        if self.mode == 'anytime':
            new_weights = self.learner.learn_anytime(self.weights)
        else:
    ...
    def evaluating(self, weights):
        ...
        if self.num_training_images > 0:
            print("Training:")
            train_errors, train_total_inconsistency = Eval.evaluate_training_images(self.images, self.models, self.labels, self.names, weights, self.num_training_images, self.inference_type, self.max_iter, self.inc, self.plot, mode=self.mode)
        ...
        if self.num_testing_images > 0:
            print("Testing:")
            test_errors, test_total_inconsistency = Eval.evaluate_testing_images(self.path+'/test/test', weights, self.num_states, self.num_testing_images, self.inference_type, self.max_iter, self.inc, self.plot, mode=self.mode)
