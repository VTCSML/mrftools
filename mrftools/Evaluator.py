...
class Evaluator(object):
    ...
    # @profile
    def evaluate_training_images(self, saved_path, saved_file_name, images, models, labels, names, weights, num_images, inference_type, max_iter= 300, inc= False, plot = True, display='display'):
        np.set_printoptions(precision=10)
        loader = ImageLoader(self.max_width, self.max_height)
    ...
    def evaluate_testing_images(self, saved_path, saved_file_name, directory, weights, num_states, num_images, inference_type, max_iter= 300, inc= False, plot = True, display = 'final'):
        ...
        images, models, labels, names = loader.load_all_images_and_labels(directory, num_states, num_images)
        if inc == True:
            average_errors, total_inconsistency = self.evaluate_training_images(saved_path, saved_file_name, images, models, labels, names, weights, num_images, inference_type, max_iter, inc, plot)
            return average_errors, total_inconsistency

        else:
            average_errors = self.evaluate_training_images(saved_path, saved_file_name, images, models, labels, names, weights, num_images, inference_type, max_iter, inc, plot)
            return average_errors
