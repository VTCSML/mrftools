...
class Learner(object):
    ...
    """
    def __init__(self, batchsize, inference_type=MatrixBeliefPropagator):
        """
    ...
    def objective(self, weights, options=None):
        ...
        #mini_batch_bp = []
        if (self.stamp + self.batchsize < self.num_examples):
            mini_batch_bp = self.belief_propagators[self.stamp, self.stamp + self.batchsize]
        else:
            mini_batch_bp = self.belief_propagators[self.stamp, self.num_examples-1]
            remain = self.batchsize - (self.num_examples - self.stamp) - 1
            mini_batch_bp_1 = self.belief_propagators[0, remain]
            mini_batch_bp = mini_batch_bp + mini_batch_bp_1

        self.inferred_expectations = self.calculate_expectations(weights, mini_batch_bp, False)
    ...
    def gradient(self, weights, options=None):
        ...
        if self.start_time != 0 and time.time() - self.start_time > self.max_time:
            ...
            #     self.calculate_expectations(weights, self.belief_propagators, False)
