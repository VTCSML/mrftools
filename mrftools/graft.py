...
def graft(variables, num_states, data, l1_coeff, MAX_ITER_GRAFT):
    """
    ...
    selected_var, max_grad, search_space = get_max_gradient(aml_optimize.belief_propagators, len(data), search_space, edges_data_sum)
    ...
    for instance in data:
        aml_optimize.add_data(instance)
    weights_opt = aml_optimize.learn(np.random.randn(aml_optimize.weight_dim), 2500)
