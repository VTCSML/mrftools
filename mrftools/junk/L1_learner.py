...
class L1_learner():
    ...
    def learn_structure(self, real_edges=list()):
        """
        ...
        self.active_set = self.search_space
        ...
        if self.is_show_metrics:
            recall, precision = self.get_metrics(real_edges)
    ...
    def remove_zero_edges(self):
        ...
        for edge in self.active_set:
            print(edge)
            length_normalizer = float(1) / np.sqrt(len(bp.mn.unary_potentials[edge[0]]) * len(bp.mn.unary_potentials[edge[1]]))
            ...
            print(length_normalizer * np.sqrt(edge_weights.dot(edge_weights)))
