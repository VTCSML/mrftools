...
class SelectiveStructuredPriorityGraft1():
    """
    ...
    def update_pq(self):
        if self.m > 0:
            ...
            for i in range(self.m):
                ...
                for node in self.not_connected.keys():
                    if self.not_connected[node] == True and (min(hub,node),max(hub,node)) in self.pq:
                        self.pq.updateitem((min(hub,node),max(hub,node)), self.centrality[hub])
