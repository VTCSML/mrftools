...
METHODS = ['best_k','queue', 'struct_best_k', 'graft' ]

METHODS = ['graft' ]
def main():
	_lambda = .01
	alpha_range = [0 , .001, .01, .1, .25, .5, .75, .9, .99] #np.arange(0,1,.01)
	node_reg = 0
	len_data = 5000
	...
	for alpha in alpha_range:
		...
		plt.savefig('../../../pq_plot/' + str(len(variables)) + '_' + str(alpha) + '_Recall-Loss.eps', format='eps', dpi=1000)
		plt.close()
