...
def main():
	...
	edge_num = 500 # Allowed max number of edges in the final network
	data_train_ratio = .8
	...
	for edge_reg in edge_reg_range:
		for node_reg in node_reg_range:
			...
			spg.on_monitor_mn() # Unable getting snapshots of partial MRFS and their graphs call spg.mn_snapshots and spg.graph_snapshots (Both are dict() with keys as time stamps)
			...
			t = time.time()
			earned_mn, final_active_set, suff_stats_list, recall, precision, f1_score, objec, _ = spg.learn_structure(edge_num) # Learn structure
			...
			print('======SPG Exec Time: ' + str(exec_time))
	...
	t = time.time()
	learned_mn, final_active_set, suff_stats_list, recall, precision, f1_score, objec, _ = grafter.learn_structure(edge_num, edges=edges)
	exec_time = time.time() - t
	graft_nll = compute_likelihood(learned_mn, len(variables), test_data) 
	print('======GRAFT NLL: ' + str(graft_nll))
	print('======GRAFT Exec Time: ' + str(exec_time))
