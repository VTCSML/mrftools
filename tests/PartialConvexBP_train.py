...
def train_model(N, models, labels, num_class, inference_type, plot_path, output_dir):
    plt.clf()
    learner = PartialLearner(N, inference_type)
...
def FCN_features_train(dir, dataset, output_dir, size, N, num_class, output_name, inference_type, plot_path):
    models, labels, names = batch_load_images_features(dir, size, dataset, num_class)
    weights = train_model(N, models, labels, num_class, inference_type, plot_path, output_dir)
