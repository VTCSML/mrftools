...
class TestMaxProductBeliefPropagator(unittest.TestCase):
    ...
    def test_consistency(self):
        """Test that max-product leads to locally consistent estimates"""
    ...
    def test_exactness(self):
        """Test that max-product infers the true MAP state in a chain."""
        mn = self.create_chain_model()
        bp = MaxProductBeliefPropagator(mn)
        bp.infer(display='full')
        bp.load_beliefs()
    ...
    def test_overflow(self):
        """Test that max product does not fail when given a large, poorly scaled potential."""
        mn = self.create_loop_model()
